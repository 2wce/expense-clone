...
export default function PlanBadge({
  plan = tierNames.basic.key,
  className = '',
  isPremiumPlanEnded
}: Props) {
  const colorClassName =
    plan === tierNames.premium.key ? 'bg-yellow-300' : 'bg-sky-300'
  return (
    <p className={`${defaultClassName} ${colorClassName} ${className}`}>
      {isPremiumPlanEnded ? 'Expired' : plan}
    </p>
  )
}
