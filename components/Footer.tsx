...
const Footer = ({ className }: { className?: string }) => {
  return (
    <footer
      className={`flex h-14 w-full items-center border-t pl-6 pr-6 font-medium ${className}`}
    >
      <div className="mb-[8px] flex w-full justify-center xs:mb-0">
        <div className="flex flex-row items-center justify-between gap-6 border-t border-white/5">
          <p className="text-xs text-gray-600">
            &copy; Copyright {new Date().getFullYear()}, Expense.fyi
          </p>
          <div className="flex gap-3">
            <Link className="group" href={siteUrls.twitterUrl}>
              <span className="sr-only">Follow us on Twitter</span>
              <svg
                viewBox="0 0 20 20"
                aria-hidden="true"
                className="h-5 w-5 fill-sky-400 transition group-hover:fill-zinc-700"
              >
            ...
            <Link className="group" href={siteUrls.githubUrl}>
              <span className="sr-only">GitHub</span>
              <svg
                viewBox="0 0 20 20"
                aria-hidden="true"
                className="h-5 w-5 fill-zinc-900 transition group-hover:fill-zinc-700"
              >
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
