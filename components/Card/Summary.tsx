...
const calculateAmountForSubscription = (
  acc: number,
  datum: { price: number; paid_dates: any[] }
) => Number(datum.price) * Number(datum.paid_dates.length) + acc
...
export default function Summary({
  ...
  const totalSubscriptionsCost =
    totalSubscriptionsCostYear + totalSubscriptionsCostMonthly
  const totalExpenseCost = expensesData.reduce(
  ...
  const totalInvestmentsAmount = investmentsData.reduce(
    (acc, datum) => Number(datum.price) * datum.units + acc,
    0
  )
  const totalBalanceAmount =
    totalIncomeAmount -
    (totalSubscriptionsCost + totalExpenseCost + totalInvestmentsAmount)
  const totalSpent =
    totalSubscriptionsCost + totalExpenseCost + totalInvestmentsAmount
