...
const preferencesLinks = [
  {
    link: 'mailto:support@expense.fyi',
    activePath: '',
    name: 'Support',
    Icon: SupportIcon
  },
...
export default function Sidebar({
  ...
  async function signOut() {
    ...
    // @ts-expect-error changing this to  nextjs primitive
  ...
  return (
    <nav
      className={`z-[1] flex min-h-full flex-col bg-zinc-900 pl-3 pr-3 pt-2 pb-2 ${
        ...
        show ? 'w-[280px] ' + overrideClassname : 'w-[64px]'
      }`}
    >
      <aside
        className={`z-[10] mb-[10px] flex h-full w-[100%] flex-col justify-between ${
          show ? 'xs:w-[260px]' : ''
        }`}
      >
        <div>
                ...
                <Tooltip.Trigger asChild>
                  <Link
                    ...
                    className={`mt-2 flex h-[40px] items-center rounded-lg p-2 text-base tracking-wide text-white transition-all hover:bg-zinc-800 ${
                      router.pathname === linkItem.activePath
                        ? 'bg-zinc-800'
                        : ''
                  ...
                  >
                    <span className="flex items-center">
                      <linkItem.Icon />
                      <span className={`ml-2 ${show ? 'visible' : 'hidden'}`}>
                        {linkItem.name}
                      </span>
                    </span>
                  </Link>
          ...
          {preferencesLinks.map((linkItem) => (
            <div key={linkItem.name} onClick={onHide}>
              ...
              >
                <span className="flex items-center">
                  <linkItem.Icon />
                  <span className={`ml-2 mt-0 ${show ? '' : 'hidden'}`}>
                    {linkItem.name}
                  </span>
                </span>
              </Link>
      ...
      {show ? (
        <div
          className="z-9 fixed inset-0 transition-opacity"
          onClick={() => onToggle()}
        >
          <div
            tabIndex={0}
