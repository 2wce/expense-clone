...
export default function Home({ user }: Props) {
  ...
  const { data: expensesData = [], isLoading: isExpensesLoading } = useSWR(
    `/api/expenses/range?${rangeParams}`
  )
  const { data: investmentsData = [] } = useSWR(
    `/api/investments/range?${rangeParams}`
  )
  const { data: incomeData = [], isLoading } = useSWR(
    `/api/income/range?${rangeParams}`
  )
  const { data: subscriptionsData = [], isLoading: isSubsLoading } = useSWR(
    `/api/subscriptions/all?${rangeParams}`
  )

  const chartdata = useMemo(
    () => extractExpensesData(expensesData, user.locale),
    [expensesData, user.locale]
  )
  const categoriesForChartData = useMemo(
    () => extractExpensesCategories(expensesData),
    [expensesData]
  )
  const [maxValueForXAxisBarChart] = useMemo(
    () => extractMaxXAxisValue(chartdata),
    [chartdata]
  )
  const topSpendExpenseCategory = useMemo(
    () => extractTopExpenseCategoryData(expensesData),
    [expensesData]
  )
  const subscriptionDataForLegend = useMemo(
    () => extractCategoriesFromData(subscriptionsData),
    [subscriptionsData]
  )

  const chartDataForSubscription = useMemo(
    () => extractSubscriptionData(subscriptionsData),
    [subscriptionsData]
  )
        ...
        <div className="mb-4 flex flex-col justify-between sm:flex-row ">
          ...
          <div className="flex items-center font-medium">
            <div className="ranger-picker mr-2 flex w-[calc(100%-51px)] items-center sm:mr-4">
              <span className="mr-2 hidden text-xs font-semibold uppercase sm:inline-block">
                Showing:
              </span>
              <DateRangePicker
                defaultValue={[
                ...
                onValueChange={(value) => {
                  ...
                  setSelectedDateObj({
                    start: format(new Date(startDate as Date), dateFormatStr),
                    end: format(new Date(endDate as Date), dateFormatStr),
                    dropdown: dropdown as string
                  })
                }}
                maxDate={new Date()}
                enableDropdown={true}
              ...
              {isSubsLoading ? (
                ...
                <>
                  <div className="flex justify-end">
                    <Legend
                      categories={subscriptionDataForLegend}
                      marginTop="mt-3"
                    />
                  </div>
                  <DonutChart
                      ...
                      <>
                        {!isLoading && topSpendExpenseCategory.length ? (
                          <div className="mt-2 flex justify-between">
                            <p className="font-default text-sm font-medium text-zinc-500">
                              Category
                            </p>
                            <p className="font-default text-sm font-medium text-zinc-500">
                              Amount
